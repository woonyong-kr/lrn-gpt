...
def load_checkpoint(model: GPTModel, optimizer: torch.optim.Optimizer | None, path: str, device: torch.device) -> tuple[int, int]:
...
def generate_and_print_sample(model: GPTModel, tokenizer, device: torch.device, start_context: str, max_new_tokens: int = 50, context_size: int = 256, temperature: float = 0.8, top_k: int | None = 40) -> None:
    ...
    out = generate(model, idx, max_new_tokens=max_new_tokens, context_size=context_size, temperature=temperature, top_k=top_k)
