...
class TestBPETokenizer:
    ...
    def test_decode_preserves_special_token_order(self):
        """skip_special=False이면 특수 토큰도 원래 위치에 맞게 복원되는지 확인한다."""
        tok = BPETokenizer(vocab_size=300)
        tok._init_special_tokens()

        ids = [SPECIAL_IDS[BOS_TOKEN], *tok.encode("A"), SPECIAL_IDS[EOS_TOKEN]]

        assert tok.decode(ids, skip_special=False) == "<bos>A<eos>"
...
class TestBPETrain:
    ...
    def test_train_respects_min_frequency(self):
        """min_frequency보다 적게 나온 pair는 병합하지 않는지 확인한다."""
        tok = BPETokenizer(vocab_size=270, min_frequency=2)
